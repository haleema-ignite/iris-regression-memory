...
describe("retrieval", () => {
  it("evaluates every matching contract by default", () => {
...
`);
    const hits = retrieve(contracts, "ignitetech-group/iris-sp-engines", diff);
    assert.ok(hits.length > 1);
    assert.equal(hits.length, retrieve(contracts, "ignitetech-group/iris-sp-engines", diff, 100).length);
    assert.ok(hits.every((hit) => hit.pathMatched || hit.interfaceMatched));
  });

  it("supports an explicit display limit without changing the default", () => {
    const contracts = loadContracts();
    const diff = parseUnifiedDiff(`diff --git a/engines/facebook/src/webhook/webhook.component.ts b/engines/facebook/src/webhook/webhook.component.ts
--- a/engines/facebook/src/webhook/webhook.component.ts
+++ b/engines/facebook/src/webhook/webhook.component.ts
...
+appSecret
`);
    assert.equal(retrieve(contracts, "ignitetech-group/iris-sp-engines", diff, 1).length, 1);
    assert.ok(retrieve(contracts, "ignitetech-group/iris-sp-engines", diff).length > 1);
  });
});
